...
def get_opponent():
    ...
    opponent_path = 'opponent.pth'
    
    if os.path.exists(opponent_path):
        try:
            opponent.load_state_dict(torch.load(opponent_path, map_location=device))
        ...
        if os.path.exists('model.pth'):
             ...
             try:
                opponent.load_state_dict(torch.load('model.pth', map_location=device))
             except Exception as e:
